...
export default function TrailsPage({ currentUser }) {
  ...
  const filteredTrails = useMemo(() => {
    return trails
      ...
      .filter((t) => (difficultyFilter ? t.Rating?.toString() === difficultyFilter : true))
      .sort((a, b) => {
        switch (sortOption) {
          case "length":
  ...
  return (
    <div className="space-y-12 px-4 py-10 max-w-screen-xl mx-auto">
      ...
      <MapView trails={boundsFilteredTrails} onBoundsChange={setMapBounds} showAll={showAllMarkers} />
      ...
      {currentUser?.id && (
        <RecommendationsPanel userId={currentUser.id} topN={6} />
      )}
  ...
  <section className="bg-purple-50 p-6 rounded-lg shadow-sm">
    <ChallengeTrailsPanel minMiles={12} minRating={4.2} limit={8} />
  </section>
